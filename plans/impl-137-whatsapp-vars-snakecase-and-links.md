...
11. **(C-5) Template name / registry.** No `WA_CONTENT_SID_PROJECT_WELCOME` constant exists; templates are name strings. Confirm `siapp_project_welcome_v1_en` (define `PROJECT_WELCOME_TEMPLATE`); note the existing collab constant is `collab_access_link_v1` (no `siapp_`/`_en`) — reconcile the naming convention with the template author.
...
16. **(F-4) Provider source not on this worktree branch.** `twilioProvider.ts` (name-keyed `buildContentVariables`) is present here only as the compiled `dist` artifact; the source ships on the merged #133 branch. Confirm origin/main carries it (Finding 1's contract assumes the merged name-keyed behaviour) before branching implementation from main.
