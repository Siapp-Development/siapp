...
1. **`.github/workflows/ci.yml`** (new)
   ...
   - Job `ci` (display name **`CI`** — this is the required-check name):
     checkout@v4 → pnpm/action-setup@v4 (version from `packageManager`) → setup-node@v4 (node version from `.nvmrc`, currently 22, `cache: pnpm`) → `pnpm install --frozen-lockfile` → `pnpm build` → `pnpm lint` → `pnpm typecheck` → `pnpm test` (root scripts fan out via turbo).
