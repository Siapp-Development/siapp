...
|---|---|---|---|
| `task_status_change` (`task_status_change_v1`) | client (± member per `notify.toInternal`) | `firm_name`, `project_title`, `task_title`, `new_status`, **`portal_token`** (NEW) | durable client link for `(wid, projectId, projectData.clientId)` via `getOrCreateClientPortalLink` — bare `token` |
| `task_blocked` (`task_blocked_v1`) | client (± member) | `firm_name`, `project_title`, `task_title`, **`portal_token`** (NEW) — **`blocked_reason` DROPPED** | same durable client link — bare `token` |

**`blocked_reason` decision — DROP it from the client-facing `task_blocked` emit.** Rationale: the #137 §5 copy decision is that the client message says the task is "on hold" and deliberately omits the internal reason (avoids leaking restricted internal content to the client). `ITaskBlockedVars` already carries no `blockedReason`, so this only removes the wire var. **Contingent on the approved template copy:** the final `task_blocked_v1` Content Template MUST NOT declare a `blocked_reason` named variable (Meta matches by name; a declared-but-unsent var breaks substitution). Since `WA_CONTENT_SID_TASK_BLOCKED` is blank and the template is not yet approved, dropping now costs zero re-approval. **Human must confirm** the approved copy has no `blocked_reason` before this ships. (If the firm later wants an internal-only reason for members, that belongs to a separate internal template — out of scope here.)
...
`portal_token` is intended for **CLIENT** recipients of these two triggers. Constraint: `templateVariables()` builds ONE shared `variables` map per event, and the dispatcher substitutes an empty variable with an em-dash `—` — so a member message with an empty `portal_token` would render a broken `https://siapp.app/p/—`.

- **Recommendation:** resolve the durable client link ONCE per event and place `portal_token` in the shared map, so it is present for the client recipient (and, if `notify.toInternal` is set, also for member recipients — who then receive the client's tracker link). This fills the variable for every recipient of the single shared client-facing template and avoids the broken em-dash URL. The mild "member sees the client tracker link" exposure is **pre-existing fan-out behavior** (#137 open Q #3), surfaced — not introduced — by link population. **Flag for human call.**
- **Alternative** (if members must be excluded): gate `toInternal` off for `task_status_change`/`task_blocked`, or author a separate member template — both are behavior/scope changes, deferred as an open question.
...
- `getOrCreateClientPortalLink` does read-before-write: query active client links (`audience=='client'`, `scopeType=='project'`, `scopeId==projectId`, `subjectId==clientId`, `revoked==false`); reuse when `expiresAt>now` AND `token` non-empty; else mint (revoke-active-then-set). Across many events (dueSoonSweep, repeated status changes) it reuses the durable link → one-active-link + D-042 preserved (in-flight links never 404).
- **Concurrent first-mint race:** two events for the same `(project, client)` with no existing link could both mint. Mitigation: keep the mint path **transactional** (the existing `mintClientPortalLink` already runs in a `runTransaction` that revokes-active + sets — mirror it in the get-or-create mint branch; do NOT copy the collaborator helper's non-transactional sequence). Optionally use the deterministic doc id `portalUid(wid,projectId,clientId)` so concurrent creates collide on one doc id for a hard idempotency guarantee. Recommend transaction (minimal change from today's client mint).
