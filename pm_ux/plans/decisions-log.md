...
3. **Visibility is PINNED to the task in rules.** Link attachments copy the task's `visibleToClient` / `restrictedToDepartments`, exactly like file uploads — so a firm-shared Drive link *can* surface in the client portal / collaborator surfaces under the firm's existing curation toggles. `validLinkDocumentCreate` reads the parent task at the submitted `scopeId` and enforces `visibleToClient == task.visibleToClient` and `restrictedToDepartments == task.restrictedToDepartments` (mirrors `validCollabDocumentCreate`), so a direct Firestore write cannot leak a client-visible Drive URL for a task whose real visibility differs. No firm-internal-only guard.
4. **Link creation is firm-only; portal/collab render links as external anchors.** Portal (`/p`) and collaborator (`/t`) surfaces may *see* inherited link rows but cannot create them (their rules branches are unchanged). Both readers carry `attachmentType`/`url`/`linkProvider` and render a link row as a safe external anchor (`target="_blank" rel="noopener noreferrer"`) with the provider label and no size/Download — never a broken Storage fetch.
